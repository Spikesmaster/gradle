...
public class CachingModuleVersionRepository implements LocalAwareModuleVersionRepository {
    ...
    public void resolve(Artifact artifact, BuildableArtifactResolveResult result) {
        ...
        if (cached != null) {
            ...
            long cacheDescriptorAge = cachedModuleDescriptor != null ? cachedModuleDescriptor.getAgeMillis() : 0;
            if (cached.isMissing()) {
                if (!cachePolicy.mustRefreshArtifact(artifactIdentifier, null, age, artifactBelongsToChangingModule, cacheDescriptorAge, descriptorHash == cached.getDescriptorHash())) {
                ...
                if (!cachePolicy.mustRefreshArtifact(artifactIdentifier, cachedArtifactFile, age, artifactBelongsToChangingModule, cacheDescriptorAge, descriptorHash == cached.getDescriptorHash())) {
